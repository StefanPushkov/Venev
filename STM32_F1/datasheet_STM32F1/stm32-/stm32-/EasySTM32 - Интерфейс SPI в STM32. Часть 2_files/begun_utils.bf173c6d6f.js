...
if (!Begun.Utils) {
	...
	Begun.Utils = new function() {
		...
		this.includeScript = function (url, type, callback, check_var) {
			...
			var w = window,
				adsDomain = w.begun_block_id ? 'begun' : w.price_block_id ? 'price' : 'begun',
				inc = 0,
				protocol = Begun.Scripts.getConformProtocol(),
				script = null,
				undefined,
				scriptTimeoutCounter = protocol +
					'//autocontext.' + adsDomain + '.ru/blockcounter?pad_id={{pad_id}}&block={{block_id}}&script_timeout=1';
						...
						w.setTimeout(function(){
							...
							if (s && w[check_var] === undefined){
								...
								Begun.Utils.includeCounter(
									...
									{
										'pad_id': (w.begun_auto_pad || w.price_auto_pad || ''),
										'block_id': (w.begun_block_id || w.price_block_id || '')
									}
								);
							}
						}, script_timeout);
